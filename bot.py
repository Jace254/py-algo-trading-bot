...
def run_bot(network: str):
    ...
    while (True):
        ...
        try:
            ...
            print(f"Error attempting to query for desired trades: {e} here2")
            sys.exit(1)

        # Create Tinyman client on behalf of the trading wallet address.
        if token_network == "testnet":
            client = TinymanTestnetClient(
                algod_client, user_address=account.address)
        else:
            client = TinymanMainnetClient(
                algod_client=algod_client, user_address=account.address)
