...
CREATE TABLE `assets` (
  ...
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=0 DEFAULT CHARSET=utf8 COMMENT='Holds information on the native Algo token and ASAs that are supported by the trading bot.';
...
CREATE TABLE `trades` (
  ...
  `token_network` varchar(8) NOT NULL,
  `is_active` tinyint(1) NOT NULL DEFAULT 1,
  `origin_trade` int(10) unsigned NULL,
  PRIMARY KEY (`id`),
...
) ENGINE=InnoDB AUTO_INCREMENT=0 DEFAULT CHARSET=utf8 COMMENT='Holds records of trades that we want to transact and the conditions with which they are allowed, as well as a flag to indicate when the trades are successfully executed.';